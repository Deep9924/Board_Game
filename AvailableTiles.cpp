...
{
	unsigned int chosen = rand() % 12;
	if(chosen < 6)
	{
		unsigned int dice = 1 + rand() % 2;
		TileDice tileDice(dice);
		return new AvailableTilesElement(tileDice, 5 * dice);
	}
	else if (chosen < 10)
	{
		unsigned int points = 1 + rand() % 3;
		TilePoints tilePoints(points);
		return new AvailableTilesElement(tilePoints, points * points);
	}
  else 
  {
		TileBomb tileBomb;
